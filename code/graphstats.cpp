...
int graphStats(string filename, bool directed, gStats& graphStatistics, bool debug){
    ...
    // calculate density as edges/nodes(nodes-1) if directed, 2edges/nodes(nodes-1) if undirected,
    graphStatistics.density=(float)graphStatistics.nEdges/(float)(((long)graphStatistics.nNodes*((long)graphStatistics.nNodes-1)));
    if (!directed){
        graphStatistics.density=graphStatistics.density*2;
    }
